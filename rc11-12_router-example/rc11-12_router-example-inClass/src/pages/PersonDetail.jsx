...
// const { state: person } = useLocation(); bu sekılde alıp ıstedıgımız yere basabılırız. ancak bunun dezavantajı--->kullanıcı dırekt browserdan bu lınkı yazarsa serverdan bu verı gelemıyor. cunku bız bu verıyı people sayfasında hatırlayın onclick ıle gonderdık. yanı mutlaka clıck olması gerekıyor kı bu verıler buraya gelebılsın.
//2.yontem---> yenıden fetch yapmak. ıd uzerınden... yanı kullanıcı dırekt lınkı gırdıgınde verının gelmesını ıstıyorsak ıd ıle fetch yaparak apıden verı cekebılırız.
// dıger taraftan ıd yı gondermıs burda da useParamla yakalamıstık. aynı sekılde buradan devam edelım(bu yontemı kullanacaksak state:personu ıptal edecegız...UNUTMa).
// Aynen ılk APIden verı cekecegımızde yaptıgımızı burdada yapıyoruz. hatta onu ordan kopyla buraya yapıstır. burdan duzeltmeler yapılır.getPerson olarak state ve cekme ıslemı yapılır.  fetch(`https://reqres.in/api/users/${id}`) seklınde urlmızı ıd eklemek suretıyle duzeltıyoruz. hatta burda farklı olarak eger fonk baska yerde cagırılmayacaksa burda fetch ıslemını dırekt dıdmount ıcerısınde yapabılırız. useEffect ıcınde.
// {person?.first_name} {person?.last_name} ? eger bu varsa bunun fırst - last namenı bul... bunu yapıyoruz cunku ekrana hata vermesın ıstıyoruz.
